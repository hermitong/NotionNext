...
export async function generateRss(props) {
  ...
  const TITLE = siteInfo?.title || '行走无声'
  const DESCRIPTION = siteInfo?.description || '做一个世界的水手，去所有未经的码头'
  const LINK = siteInfo?.link || 'https://www.hermit.com'
  const AUTHOR = NOTION_CONFIG?.AUTHOR || BLOG.AUTHOR || '行走无声'
  const LANG = NOTION_CONFIG?.LANG || BLOG.LANG || 'zh-cn'
  const SUB_PATH = NOTION_CONFIG?.SUB_PATH || BLOG.SUB_PATH || 'rss'
  const CONTACT_EMAIL = NOTION_CONFIG?.CONTACT_EMAIL || BLOG.CONTACT_EMAIL || 'zthopen@126.com'
